# KERYX (SIAG): Especificación Funcional y Arquitectura Cognitiva

## 1. El Propósito (Visión)
KERYX (SIAG) no es un publicador pasivo. Es un **Motor Generativo de Autopublicación (Generative Publishing Engine)**. Su misión es tomar una "Idea Semilla" y, mediante algoritmos de inteligencia artificial, re-empaquetarla, variarla (spintax) y distribuirla a través de múltiples cuentas sin penalizaciones por spam, manteniendo la resiliencia absoluta de la conexión.
...
### 2.2 Motor Generativo (Spintax y Variación)
...
* **Función:** Capa de abstracción. KERYX (SIAG) nunca habla directo con Facebook. KERYX (SIAG) habla con la interfaz `ISocialAdapter`, enviándole el payload estandarizado (`RichTextSegment`, `MediaAsset`). El Adaptador traduce esto a la API específica.
...
1. **Inteligencia Nativa, no como Add-on**: Las herramientas tradicionales agregaron un botón de "Generar con IA". KERYX (SIAG) tiene la IA en el centro del flujo de trabajo (Generación Masiva -> Rotación de Keys -> Spintax).
2. **Spintax Semántico Automatizado**: Publicar el mismo mensaje en 5 páginas te hace shadowban. KERYX (SIAG) genera 5 mensajes únicos que dicen lo mismo, programándolos al instante.
3. **Resiliencia Extrema (Military Grade)**: Si cae la API de IA, KERYX (SIAG) cambia de proveedor en nanosegundos sin intervención humana.

## 5. Roadmap Hacia el Grado Militar Mundial (The "Anti-Ban" Architecture)
Para evitar que el sistema sea etiquetado como "Botnet" por Meta, X o TikTok, el roadmap de KERYX (SIAG) requiere implementar las tácticas de las herramientas de *Guerilla Marketing* y agencias globales:
...
1. **Proxy Rotation & IP Anchoring:**
   ...
   - **Solución KERYX (SIAG):** Modificar `ISocialAdapter` para que requiera un objeto `ProxyConfig`. Cada cuenta social se amarra a un proxy residencial geolocalizado. 
2. **Account Warm-up Algorithms:**
   - **Problema:** Las cuentas jóvenes mueren por publicar agresivamente.
   - **Solución KERYX (SIAG):** Implementar en `BullMQ` un *Ramp-up Scheduler*. Si la cuenta es nivel 1 (nueva), solo publica 1 vez al día. A los 30 días sube a nivel 2.
